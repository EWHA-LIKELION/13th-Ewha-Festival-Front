import * as React from 'react';
const SvgTrash = ({ stroke = 'white', ...props }) => (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    width='20'
    height='20'
    viewBox='0 0 20 20'
    fill='none'
    {...props}
  >
    <path
      d='M17.5 4.98307C14.725 4.70807 11.9333 4.56641 9.15 4.56641C7.5 4.56641 5.85 4.64974 4.2 4.81641L2.5 4.98307'
      stroke={stroke}
      strokeLinecap='round'
      strokeLinejoin='round'
    />
    <path
      d='M7.08337 4.14199L7.26671 3.05033C7.40004 2.25866 7.50004 1.66699 8.90837 1.66699H11.0917C12.5 1.66699 12.6084 2.29199 12.7334 3.05866L12.9167 4.14199'
      stroke={stroke}
      strokeLinecap='round'
      strokeLinejoin='round'
    />
    <path
      d='M15.7083 7.61719L15.1666 16.0089C15.075 17.3172 15 18.3339 12.675 18.3339H7.32496C4.99996 18.3339 4.92496 17.3172 4.83329 16.0089L4.29163 7.61719'
      stroke={stroke}
      strokeLinecap='round'
      strokeLinejoin='round'
    />
    <path
      d='M8.6084 13.75H11.3834'
      stroke={stroke}
      strokeLinecap='round'
      strokeLinejoin='round'
    />
    <path
      d='M7.91663 10.417H12.0833'
      stroke={stroke}
      strokeLinecap='round'
      strokeLinejoin='round'
    />
  </svg>
);
export default SvgTrash;
